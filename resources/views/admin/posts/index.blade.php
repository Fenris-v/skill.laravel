        ...
        <div class="col">
            ...
            <ul class="d-flex flex-wrap list-unstyled">
                ...
                @foreach(\App\Models\Tag::all() as $tag)
                    <li class="mr-3 mb-2">
                        <a class="btn btn-primary {{ url()->current() === route('admin.posts.tag', $tag->slug) ? 'active' : '' }}"
                           href="{{ route('admin.posts.tag', $tag->slug) }}">{{ $tag->name }}</a>
                    </li>
                @endforeach
            </ul>
        </div>
    ...
    <table class="table table-bordered">
        ...
        @foreach($posts as $post)
            <tr>
                ...
                <th class="font-weight-normal" scope="col"><a href="{{ route('admin.posts.edit', $post->slug) }}">Изменить</a></th>
            </tr>
        @endforeach
        </tbody>
    </table>
